...
def loggedUser_gameLanding(user_alias):
    session['current'] = '/' + \
        str(current_user.alias) + '/have-fun-with-my-game/'
...
def loggedUserPlay(user_alias):
    session['current'] = '/' + \
        str(current_user.alias) + '/have-fun-with-my-game/play/'
...
def answerKey():
    flash("Attention! Please complete the game before see the answer key",
          category='warning')
...
def submit():
    ...
    if request.method == 'POST':
        ...
        if (current_user.is_authenticated):
            ...
            try:
                pts = Game(finish_at=datetime.now(), score=corNum,
                           played_by=current_user.fname)
                db.session.add(pts)
                db.session.commit()
                flash(
                    f"Good Job！You got {corNum}/10 points!", category='success')
            ...
            flash(
                f"Good Job！You got {corNum}/10 points! (Please note that guest user doesn't have score saving record)", category='success')
            return redirect(url_for('game.baseLandingForGame', user=current_user))
    return redirect(url_for('game.baseLandingForGame', user=current_user))
